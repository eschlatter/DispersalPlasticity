...
experiment_index$target_dist_name <- case_match(experiment_index$target_dist,"identity"~"Normally-distributed","C"~"Bimodal","D"~"Mostly bad","E"~"Mostly good")

df_all <- list()
abund_df <- list()
elt_i <- 1
max_t <- data.frame(subex=character(),repID=integer(),mapID=integer(),maxt=integer())

#subexperiment_names <- c("disturb_frequent","disturb_frequent_b1000","disturb_frequent_b01")
subexperiment_names <- c("common_disturb_b01","rare_disturb_b01","no_disturb_b01")
#subexperiment_names <- "smallkern_disturb"
...
for(subexp_i in subexperiment_names){
  ...
  for(repID_i in 1){
    ...
    for(i in 1:length(v_maps)){
      mapID_i <- v_maps[i]
      if(file.exists(paste0(output_folder,"/rep",repID_i,"/map_",mapID_i,"_raw.csv"))){
        dat_out <- read.csv(paste0(output_folder,"/rep",repID_i,"/map_",mapID_i,"_raw.csv"))
        dat_out$mapID <- mapID_i
        dat_out$repID <- repID_i
        dat_out$subex <- subexp_i
        if(is.null(dat_out$mean)) dat_out$mean=NA
        if(is.null(dat_out$var)) dat_out$var=NA
        ...
        max_t[elt_i,] <- data.frame(subex=subexp_i,repID=repID_i,mapID_i,maxt=max(dat_out$t_i))
...
#df_all$range[df_all$subex!="no_disturb"] <- NA

plot_metrics <- filter(df_all,t_i>50) |>
  group_by(mapID,repID,subex,metric) |> # one value per sim (and metric)
  summarize(mean=mean(mean),q25=mean(q25),q75=mean(q75),range=first(range),subex=first(subex)) |> # mean values across timesteps
  ...
  labs(x="scale of habitat quality autocorrelation (m)",y=NULL,
       title="State variables",
       color="Disturbance")

plot_corrs <- filter(df_all,t_i>50) |>
  group_by(mapID,repID,subex,metric) |> # one value per sim (and metric)
  summarize(corr_q=mean(corr_q),sd_corr_q=sd(corr_q),range=first(range),subex=first(subex)) |>
  ...
  facet_wrap(vars(metric),nrow=3,
             labeller = as_labeller(c("p"="Plasticity\n(kern response\nto best habitat)","theta"="Fundamental\nKernel Mean\n(km)","efftheta"="Effective\nKernel Mean\n(km)")))+
  scale_color_discrete(labels=c("common_disturb"="Small and Frequent (3% every timestep)","rare_disturb"="Large and Infrequent (30% every 10 timesteps)","no_disturb"="None"))+
  theme(legend.position="bottom",legend.direction="vertical", strip.text = element_blank(),strip.background=element_blank())+
  labs(x="scale of habitat quality autocorrelation (m)",y=NULL,
       title="Correlation of state variables\nwith habitat quality",color="Disturbance")

allplot <- grid.arrange(plot_metrics,plot_corrs,ncol=2) # 750x550
...
### break up by hab quality distribution, not disturbance
plot_metrics <- filter(df_all,t_i>500) |>
  group_by(mapID,target_dist_name,metric) |> # one value per sim (and metric)
  summarize(mean=mean(median),q25=mean(q05),q75=mean(q95),range=first(range_ident)) |> # mean values across timesteps
  ggplot(aes(x=range,y=mean,color=target_dist_name))+
  geom_point(position = position_dodge(width=0.1))+
  geom_errorbar(aes(ymin=q25,ymax=q75),position = position_dodge(width=0.1),width=0.2)+
  geom_hline(data=filter(dat_out,metric=="p"),aes(yintercept=1),linetype="dashed")+
  scale_x_log10()+
  facet_wrap(vars(metric),nrow=3,scales="free", strip.position = "left",
             labeller = as_labeller(c("p"="Plasticity\n(kern response\nto best habitat)","theta"="Fundamental\nKernel Mean\n(km)","efftheta"="Effective\nKernel Mean\n(km)")))+
  ggh4x::scale_y_facet(PANEL %in% c(1),trans="log10",breaks = breaks_log(base=(v_thetas[2]/v_thetas[1])),
                       labels = label_log(base=(v_thetas[2]/v_thetas[1])))+
  scale_color_discrete(palette = "Okabe-Ito")+
  theme(legend.position="bottom",legend.direction="vertical",strip.placement = "outside",strip.text = element_text(size = 11))+
  labs(x="scale of habitat quality autocorrelation (m)",y=NULL,
       title="State variables",
       color="Habitat quality distribution")

plot_corrs <- filter(df_all,t_i>500) |>
  group_by(mapID,target_dist_name,metric) |> # one value per sim (and metric)
  summarize(corr_q=mean(corr_q),sd_corr_q=sd(corr_q),range=first(range_ident)) |>
  ggplot(aes(x=range,y=corr_q,color=target_dist_name))+
  geom_point(position = position_dodge(width=0.1))+
  geom_errorbar(aes(ymin=corr_q-sd_corr_q,ymax=corr_q+sd_corr_q),position = position_dodge(width=0.1),width=0.2)+
  geom_hline(aes(yintercept=0),linetype='dashed',alpha=0.5)+
  scale_x_log10()+
  ...
  theme(legend.position="bottom",legend.direction="vertical", strip.text = element_blank(),strip.background=element_blank())+
  labs(x="scale of habitat quality autocorrelation (m)",y=NULL,
       title="Correlation of state variables\nwith habitat quality",color="Habitat quality distribution")
allplot <- grid.arrange(plot_metrics,plot_corrs,ncol=2) #730x650
ggsave(filename=paste0(experiment_folder,"/output/smallerkern_disturb/rep2/plots/allplot_",format(Sys.time(),"%Y%b%d_%H%M"),".png"),
       plot=allplot,
       width=2400,
       height=2400,
       units="px")
...
subexp_label <- "Frequent Disturbance, Moderate Fecundity"

subexp_i <- "disturb_frequent_b01"
subexp_label <- "Frequent Disturbance, Low Fecundity"

subexp_i <- "disturb_rare_b01"
subexp_label <- "Rare Disturbance, Low Fecundity"

subexp_i <- "no_disturb_b01"
subexp_label <- "No Disturbance, Low Fecundity"
...
for(i in sims_to_plot){
  ...
  g_median <- ggplot(dat_out,aes(x=t_i,group=repID,color=repID))+
    ...
    geom_ribbon(aes(ymin=q25,ymax=q75,color=NULL,fill=repID),alpha=0.2)+
