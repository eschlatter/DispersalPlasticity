...
alpha_start <- 2 # index (in v_alphas) of shape parameter initial value
theta_start <- 2 # index in (v_thetas) of scale parameter initial value
...
#n_good <- 3
K <- matrix(3, nrow=ny,ncol=nx)
#K[c(1,20,21,30)] <- 8
#K[sample(nx*ny,n_good)] <- 8
...
```{r}
matrix_out <- f_RunMatrixSim(nx,ny,nsteps,v_alphas,v_thetas,v_p,alpha_start,theta_start,p_start=2,mu,b,b_bad=1,b_neutral=3,b_good=6,K,heatmap_plot_int = 500, competition_method = 'sample')
...
```
```{r}
matrix_out <- f_RunMatrixSim(nx,ny,nsteps,v_alphas,v_thetas,v_p,alpha_start,theta_start,p_start=2,mu,b,b_bad=1,b_neutral=3,b_good=6,K,heatmap_plot_int = 500, competition_method = 'rnorm')
f_PlotOutput(matrix_out[[2]], kern_timesteps=seq(from=0.75*nsteps,to=nsteps,length.out=25))
f_PlotAllHeatmaps(matrix_out[[1]],matrix_out[[3]])
#save(K, IBM_out,file='model_runs/20251017_1.RData')
```
...
IBM_out <- f_RunIBM(nx,ny,nsteps,v_alphas,v_thetas,v_p,alpha_start,theta_start,p_start=2,mu,b,b_bad=1,b_neutral=3,b_good=6,K,heatmap_plot_int = 500)
...
v_p <- seq(from=0,to=0.9,length.out=2) # values the plasticity parameter can take
p_start <- 2 # index (in v_p) of plasticity parameter initial value
...
for(i in 1:nrow(sims)){
  ...
  matrix_out <- f_RunIBM(nx,ny,nsteps,v_alphas,v_thetas,v_p,sims$alpha_start[i],sims$theta_start[i],p_start,mu,b,b_bad,b_neutral,b_good,K,heatmap_plot_int=500)[[2]] %>%
    mutate(sim_num=sims$sim_num[i])
  
  df_out <- rbind(df_out,matrix_out)
  sim_list[[i]] <- matrix_out
}
...
```{r}
IBM_out <- f_RunIBM(nx=20,ny=20,nsteps=100,v_alphas,v_thetas,v_p,alpha_start,theta_start,p_start,mu,b,K=5,heatmap_plot_int = 500)

IBM_out[[4]]
f_PlotAllHeatmapsIBM(IBM_out[[1]],IBM_out[[3]])
f_PlotOutput(IBM_out[[2]], kern_timesteps=seq(from=750,by=50,to=1000),kern_xlim = 25)
