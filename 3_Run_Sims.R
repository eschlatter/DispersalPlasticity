...
load(file="seascapes/hab_params_1.RData")
...
v_p <- seq(from=-1,to=1,length.out=5)# values the plasticity parameter can take
...
#### Run sim
sim_out2 <- f_RunSim(params,hab_params=make_hab_out,output_flag="all",show_plot = FALSE)

# plotting for output_flag="full"
sim_melt <- f_ProcessPij(sim_out2$Pij,patch_locations = make_hab_out$patch_locations,group_index = sim_out2$group_index)$sim_melt
by_t <- f_ProcessPij(sim_out2$Pij,patch_locations = make_hab_out$patch_locations,group_index = sim_out2$group_index)$by_t
f_PlotAllHeatmaps(sim_melt,make_hab_out$patch_locations)

# plotting for output_flag="lite"
ggplot(filter(sim_out2$df_out,t_i %in% seq(from=1,to=max(sim_out2$df_out$t_i),by=1)),aes(x=t_i,y=mean))+
  geom_line()+
