...
```{r}
nx <- 15 # size of space in the x dimension
ny <- 15 # size of space in the y dimension
...
n_good <- 3
b <- matrix(4, nrow=ny,ncol=nx)
b[sample(nx*ny,n_good)] <- 8
...
```{r}
matrix_out <- f_RunMatrixSim(nx,ny,nsteps,v_alphas,v_thetas,v_p,alpha_start,theta_start,p_start,mu,b,K,heatmap_plot_int = 500, competition_method = 'rnorm')
...
IBM_out <- f_RunIBM(nx,ny,nsteps,v_alphas,v_thetas,v_p,alpha_start,theta_start,p_start,mu,b,K,heatmap_plot_int = 500)
