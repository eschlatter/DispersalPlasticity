  ...
  for(t in 2:nsteps){
    ...
    for(i_alpha in 1:length(v_alphas)){
      for(i_theta in 1:length(v_thetas)){
        ...
        for(i_patch in 1:npatch){
          cell_popsize <- after_mutation[i_patch,i_alpha,i_theta,p_start]
          
          # dispersal
          sim_array[,i_alpha,i_theta,p_start,t] <- b*cell_popsize*cm[i_patch,] + sim_array[,i_alpha,i_theta,p_start,t]
