...
n_basemaps <- 5 # number of basemaps to use
n_k_per_base <- 5 # number of Kmaps to produce, for each basemap + h value combination
n_rep_per_k <- 5 # number of identical (up to stochasticity) reps to do of each map
...
BaseMaps <- list()
for(i in 1:3){
  hab <- f_GenerateMapWithK(base_map=NULL,K_range=c(1,1),h=0,k=7,p=0.3,h_base=0.9,plot_flag=FALSE)
...
## habitat maps 
KMaps <- list()
BaseKMapIndex <- data.frame(Kmap=1:(length(BaseMaps)*length(h_vals)*n_k_per_base),BaseMap=NA,h=NA) #keep track of which base map belongs with each kmap
ind <- 0
...
save(sim_index,file="experiments/Test_Array1_SimIndex.RData")
