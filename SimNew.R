...
refmat_file="output/TryNew_1/RefMat_10k.RData"
nsteps=100
...
#prof_sim <- profvis({
  ...
  pop_df <- data.table(patch=sample(patch_locations$id,0.75*nrow(patch_locations),replace = FALSE),
                       theta=1,
                       p=0)
  ...
  for(t_i in 1:nsteps){
    ...
    for(adult_i in 1:nrow(pop_df)){
      ...
      # get its theta value (eff_th)
      eff_th <- pop_df$eff_theta[adult_i]
      ...
      larvae_out[,adult_i] <- pmax(patch_locations$b[origin_patch]*(wt_lower*ref_mat[patch_dists_ref[,origin_patch],l_th_ind]+
    ...
    comp_results <- foreach(i_patch = will_have_vacancy) %do% {
      ...
      if(n_setts>0){ # if the patch isn't empty
        ...
        new_pop_i <- data.table(parent=survivors, patch=i_patch)
      } else new_pop_i <- NULL
...
# htmlwidgets::saveWidget(prof_sim,"output/TryNew_1/profile.html")
# browseURL("output/TryNew_1/profile.html")
#
# #### Process output ####
# library(dplyr)
# library(ggplot2)
