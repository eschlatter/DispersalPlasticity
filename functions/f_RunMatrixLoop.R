...
########################################################################
# version that only keeps the by_t info
f_RunMatrixLoopLessMem <- function(params){
  starttime <- proc.time()
  ...
  time_run <- proc.time()-starttime
  return(list(params=params,by_t=by_t,group_index=group_index,time_run=time_run,patch_locations=patch_locations))
}
