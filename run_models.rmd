...
```{r}
nx <- 13 # size of space in the x dimension
ny <- 7 # size of space in the y dimension
...
v_thetas <- seq(from=0.01,to=10,length.out=10) # values the scale parameter can take
...
```{r}
matrix_flat_out <- f_RunMatrixSimFlat(nx,ny,nsteps,v_alphas,v_thetas,v_p,alpha_start,theta_start,p_start,mu,b,b_bad,b_neutral,b_good,K,competition_method = 'sample')
...
#f_PlotAllHeatmaps(matrix_flat_out$sim_melt,matrix_flat_out$patch_locations)
```
