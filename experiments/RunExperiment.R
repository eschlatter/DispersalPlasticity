...
notes <- paste0("Basemap ",basemapID," experiment, larger mutation magnitude")   #can include a character string here with notes on the sim
load(paste0("New/df_experiments_b",basemapID,".RData"))

# choose a few qmaps to use
load("New/habs_to_use_090926.RData")
df_experiments <- dplyr::filter(df_experiments,hab_id %in% habs_to_use)
exp_i <- df_experiments[repID,]
rm(df_experiments,df_qmaps)
experimentID <- exp_i$experiment_id

# bio parameters
mutation_type <- "common"
