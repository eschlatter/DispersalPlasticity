### Precalculate objects that are specific to a popmap (set of anemone locations):
###   v_dist_bins, v_theta_bins, patch_dists_ref, offmap_corrections
...
task_i <- as.numeric(Sys.getenv("SLURM_ARRAY_TASK_ID"))
popmapIDs <- c(4,10,9,7)
popmapID <- popmapIDs[task_i]

popmapID <- 1
popmap_row <- read.csv("maps/index_popmaps.csv") |> 
...
patchdist_file <- paste0("maps/b",basemapID,"/patchdists_b",basemapID,"_p",popmapID,".RData")
output_file <- paste0("maps/b",basemapID,"/PopmapObjects_b",basemapID,"_p",popmapID,"_new.RData")
load(patchdist_file)
...
patch_dists_ref <- foreach(col_i = patch_dists,.combine="cbind") %do% {
...
base_rast <- rast(paste0("maps/b",basemapID,"/base_b",basemapID,".tif"))
map_ext <- terra::ext(base_rast)
map_bounds <- list(x=c(as.numeric(map_ext[c(1,2)])/1000),
                   y=c(as.numeric(map_ext[c(3,4)])/1000))
load(paste0("maps/b",basemapID,"/pop_b",basemapID,"_p",popmap_row$popmap_id,".RData"))
